...
export default function RallyForm({ slotsLeft, limit }: { slotsLeft: number; limit: number }) {
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-8 animate-[fadeIn_0.4s_ease]">
      ...
      <div className={`flex items-center justify-center gap-2 rounded-xl px-4 py-3 mb-6 text-sm font-semibold ${
        isFull ? "bg-red-50 text-red-600" : slotsLeft <= 2 ? "bg-orange-50 text-orange-600" : "bg-green-50 text-green-700"
      }`}>
        {isFull ? "❌ ที่นั่งเต็มแล้ว" : `✅ ที่นั่งว่าง ${slotsLeft} / ${limit} ที่`}
      </div>

      {isFull ? (
        <div className="text-center py-6 bg-gray-50 rounded-2xl">
          <div className="text-4xl mb-3">😔</div>
          <p className="text-gray-600 font-medium">ขออภัย มีผู้ลงทะเบียนครบแล้ว</p>
          <p className="text-gray-400 text-sm mt-1">กรุณาติดต่อผู้ดูแลชมรม</p>
        </div>
      ) : status === "success" ? (
